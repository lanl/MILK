...
def get_arguments():
    ...
    if not debug:
        ...
        if 'import-csv' in sys.argv:
            ...
            parser.add_argument("--maud-detectors",
                                default=None, help=argparse.SUPPRESS)
            parser.add_argument("--esg-files", default=None,
                                help=argparse.SUPPRESS)
            parser.add_argument("--poni-files", default=None,
                                help=argparse.SUPPRESS)
        else:
    ...
    parser.add_argument("--omega-do-update", type=str2bool, default=False,
    ...
    parser.add_argument("--maud-run-import", type=str2bool, default=True,
                        help="If true the import will be run using the MILK batch interface.")
    ...
    if debug:
        # opts.import_csv="import.csv"
        opts.maud_input_par = "texture/poni_initial.par"
        opts.import_csv = None
        opts.maud_output_par = "analysis/Pu_samA_texture/Initial.par"
        opts.maud_detectors = ["det1", "det2"]
        opts.esg_files = ["analysis/Pu_samA_texture/caked/Pu_samA_texture_scan_0000*.edf_sum_norm_0_ge1.esg",
                          "analysis/Pu_samA_texture/caked/Pu_samA_texture_scan_0000*.edf_sum_norm_0_ge2.esg"]
        #opts.poni_files = ["calibrations/ge1.poni", "calibrations/ge2.poni"]
        opts.poni_files = None
        opts.omega_start = 0.0
        opts.omega_step = 20.0

    opts.cwd = Path(opts.maud_output_par).parent

    # Check for poni_files
    opts.omegas, opts.esg_files, opts.poni_files, opts.maud_detectors = parse_opts(
        opts)
    return opts
...
def parse_opts(opts):
    if opts.import_csv is not None:
...
def main():
    ...
    if opts.maud_run_import:
        ...
        MILK.MAUDText.callMaudText.run_MAUD(
            ...
            "mx8G",
            'True',
            opts.cwd.absolute() / opts.maud_batch_script
        )
