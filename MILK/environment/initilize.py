...
def loadini(fname):
    ...
    if config.datatype.data_format=='HIPPO':
        ...
        assert len(config.hippo.omega_meas)==len(config.hippo.chi_meas),'Length omega_meas must be same as chi_meas and phi_meas'
        assert len(config.hippo.omega_meas)==len(config.hippo.phi_meas),'Length omega_meas must be same as chi_meas and phi_meas'
        assert len(config.hippo.omega_meas)==len(config.hippo.rot_names),'Length omega_meas must be same as rot_name'
        assert len(config.hippo.detectors)==len(config.hippo.banks),'Length banks must be same as detectors'
        assert len(config.hippo.detectors)==len(config.hippo.dspacing),'Length banks must be same as dspacing ranges'
        ...
        if not myfile.exists():
            ...
            resource_path=os.path.join(os.path.dirname(__file__),'resources/templates/HIPPO',fname)
